...
## 📝 Descrição do Lab
Este laboratório da **PortSwigger Web Security Academy** contém uma vulnerabilidade de **Path Traversal** (também conhecida como Directory Traversal) na exibição de imagens de produtos.

## 🎯 Objetivo
Explorar a vulnerabilidade para ler o conteúdo do arquivo `/etc/passwd` no servidor web.

## 🛠️ Ferramentas Utilizadas
**Burp Suite Professional/Community** (Intercepting Proxy & Repeater) [cite: 28]

## 🛡️ Vulnerabilidade: Path Traversal
[cite_start]A aplicação recebe um parâmetro de nome de arquivo (`filename`) para carregar imagens. Como o servidor não realiza a **sanitização de inputs** nem valida o caminho do arquivo, é possível navegar pela estrutura de diretórios do sistema operacional.

### Passo a Passo da Exploração
1. **Interceptação:** Utilizei o **Burp Suite** para interceptar a requisição HTTP que carregava uma imagem do produto.
